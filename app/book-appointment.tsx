  ...
  if (!selectedDoctor) {
    return (
      <SafeAreaView className="flex-1 justify-center items-center px-6">
        ...
        <TouchableOpacity onPress={() => router.back()} className="mt-6 bg-blue-600 px-6 py-3 rounded-full">
      ...
      <View className="p-4">
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <View className="border border-blue-100 bg-white rounded-lg mt-4 px-4 py-8 mb-8">
            <Text className="font-bold text-xl text-gray-800 mb-4">Selected Service & Doctor</Text>
            <View className="flex flex-row items-center gap-4">
              <Image
              ...
              <View className="flex-1">
                <Text className="text-gray-800 text-lg font-bold">{selectedDoctor.name}</Text>
                <Text className="text-gray-600 font-medium">{selectedDoctor.speciality}</Text>
                <Text className="text-gray-600 font-medium">{selectedDoctor.qualification || selectedDoctor.degree}</Text>
                <Text className="text-gray-600 font-medium">Experience: {selectedDoctor.experience}</Text>
                <Text className="text-gray-800 font-bold mt-3">Fee: Rs {selectedDoctor.fees}</Text>
              </View>
            </View>
            <TouchableOpacity onPress={() => router.push("/alldoctors")} className="mt-4 self-start">
              <Text className="text-blue-600 text-lg font-bold">Change</Text>
            </TouchableOpacity>
          </View>
          <DateFormat doctorId={selectedDoctor._id} />
    </SafeAreaView>
  );
}
